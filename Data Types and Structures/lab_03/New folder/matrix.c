...
double sumOfMatrixStandart(struct SparseMatrix *m1, struct SparseMatrix *m2, int rows, int columns)
{
    ...
    for (int i = 0; i < rows; i++)
        for (int j = 0; j < columns; j++) matrix1[i][j] = getElemet(m1, i, j);
    for (int i = 0; i < rows; i++)
        for (int j = 0; j < columns; j++) matrix2[i][j] = getElemet(m2, i, j);
    
    unsigned long tb1, te1;
    tb1 = tick();
    
    for (int i = 0; i < rows; i++)
        for (int j = 0; j < columns; j++) matrix[i][j] = matrix1[i][j] + matrix2[i][j];
